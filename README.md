...
```bash
conda create -n covid_prophet_dbx python=3.9
conda activate covid_prophet_dbx
```
...
```
This command will create your personal repository under `/Repos/<username>/covid_prophet_dbx`.
...
- To trigger the release pipeline, get the current version from the `covid_prophet_dbx/__init__.py` file and tag the current code version:
```
